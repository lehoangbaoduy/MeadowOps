...
interface MasterDataCrudProps {
  ...
  /** Unit 17a (MEADOWOPS-DOM-010): UI hint only, sourced from
   * getCurrentRole() — hides Add/Edit/Deactivate for the Analyst role.
   * The real enforcement is FastAPI's require_admin on the write routes;
   * this never substitutes for it (a stale/forged value here still 403s
   * server-side). */
  canWrite: boolean;
}
        ...
        ? [
            {
              id: "actions",
              header: "",
              cell: ({ row }: { row: { original: MasterDataRow } }) => (
            ...
            } satisfies ColumnDef<MasterDataRow>,
          ]
        : []),
    ];
    // entity/entityLabel are static per page and openEdit/toggleActive are
    // recreated every render anyway (not memoized) — only `fields`/
    // `canWrite` ever actually change the shape of these columns.
