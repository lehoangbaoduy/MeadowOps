...
/**
 * Unit 21 (MEADOWOPS-UI-003, PRD 6.13): live delivery for the Analyst's
 * chat inbox. The browser cannot go through this app's own server-side
...
export type ChatSocketFrame = {
  ...
  // link at all on every connection that receives it live (including the
  // sender's own — ChatConnectionRegistry broadcasts to every open
  // connection, itself included), since chat-inbox.tsx's handleFrame is
  // what actually wins the race against the POST response for the
