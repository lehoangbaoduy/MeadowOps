...
**Current phase:** Phase 4 — Hardening & Delivery — **complete** (U32 done 2026-09-14, U31 done 2026-09-14, U33 done 2026-09-14 — all three Phase 4 units done). Phase 3 — Full Simulation Loop — complete. U21a (chat delivery infrastructure) done, backend-only — see §6/DD-30. U21 (chat inbox/composer, full-stack) done — see §6/DD-31. U22 (Claude scenario generation + validation pipeline) done, backend-only — see §6/DD-32. U23 (persona-roleplay pushback suggestion + AI sufficiency check) done, backend-only — see §6/DD-33. U24 (Decision & Event Ledger full lifecycle wiring + callback lookup) done, full-stack — see §6/DD-34. U25 (AI evaluation framework + adaptive difficulty engine) done, backend-only — see §6/DD-35. U26 (human review workflow mechanism + portfolio export) done, backend-only — see §6/DD-36. U27 (warehouse transfers + carrier variability) done, backend-only — see §6/DD-37. U28 (admin SQL query history view) done, full-stack — see §6/DD-38. U29 (QA test-analyst harness — all 6 scenario types incl. callback) done, test-only — see §6/DD-39. U30 (edge case catalog implementation sweep, 29/32 rows closed at the time) done, backend-only — see §8/DD-40. U30a (chat notifications + deadline tracking, closes catalog rows 15/30) done, full-stack — see §6/DD-41. U30b (chat composer draft persistence, closes catalog row 31) done, full-stack — see §6/DD-42. U30c (chat file attachments, closes B12 entirely) done, full-stack — see §6/DD-43. Catalog row 32 (backup restore) closed 2026-09-13 via B1 (Railway+Neon deployment target) and B2 (live backup/restore drill) — edge-case catalog now 32/32, **Phase 3 exit criterion satisfied**. Phase 2 committed to git (`277b817`); Phase 3 committed to git (`dad7ef3`, 2026-09-13, 138 files) per the user's go-ahead. **Phase 4 now underway**: U32 (deployment hardening — R2 attachment storage, Postgres 18 parity, GitHub disconnect, CI wiring) done 2026-09-14, committed to git (`576df5f`/`b662ae6`) and pushed. **U31 (acceptance checklist + docs) now done, 2026-09-14** — see §9/§10 for the full checklist closure: a real committed Playwright E2E suite (the user's explicit choice over a walkthrough-script-only interpretation of PRD 9.3), the repo-root structural checks newly wired into `backend-ci.yml`, a fresh demo-data sweep, the sandbox permission boundary re-verified live, and the four generated/authored docs (`docs/schema-diagram.md`, `docs/data-dictionary.md`, `docs/data-flow.md`, `docs/testing-report.md`) plus `docs/walkthrough-script.md`. **U33 (backup automation) now done, 2026-09-14** — see §2's U33 row for the full detail: PRD 8.6's automated-daily-backup requirement, satisfied by a new `.github/workflows/backup.yml` (daily `pg_dump` of all four real schemas to a dedicated R2 bucket, integrity-checked before upload), live-verified via the GitHub API after fixing two real bugs a first attempt didn't catch (a wrong `pg_restore --list` grep pattern, caught locally before ever pushing; a Postgres 16-vs-18 `pg_dump` version mismatch, caught by an actual failed live run). **Phase 4 is now complete — all three units (U31, U32, U33) done.** **U34 (evaluation/human-review/portfolio/Ledger-action UI) done, 2026-09-14** — see §2's U34 row: not required by the PRD's own Definition of Done (§6.9/§6.10 are mechanism-only, UI deferred to Active Use), scoped anyway per the user's explicit choice to close the frontend gap U31's own walkthrough-script writeup had flagged. Four surfaces: Ledger write actions on orbynadmin's Activity page, evaluation/human-review/portfolio-export on subsystem_2's scenario detail page, and the Analyst's portfolio reflection form on orbynadmin's Chat page. Verified statically first (tsc, lint, production build, backend contract read from source), then live in a same-day follow-up pass — 7 new Playwright specs (20 total, up from 13) run repeatedly against a real booted stack, catching a real accessibility bug (a mislabeled Textarea) and a real backend finding (`app.state.claude_client` is hardcoded `None` unconditionally, so `/complete`/`/regenerate` always 503 regardless of `ANTHROPIC_API_KEY` — see §2's U34 row).
...
| Phase 4 — Hardening & Delivery | **Complete — U32 done (2026-09-14), U31 done (2026-09-14), U33 done (2026-09-14). All three Phase 4 units done.** |
...
|---|---|---|---|
| B1 | Deployment target (PRD 8.2) — small always-on host (Railway/Render/Fly.io) + managed Postgres (Supabase/Neon/Railway PG) | Needs a user account/choice + credentials; cannot self-provision | **Closed 2026-09-13.** User created a Railway Hobby account + Neon free-tier Postgres project and supplied both a Neon DSN and a Railway project token (2026-09-13). Deployed: `backend/Dockerfile` + `backend/railway.toml` written (none existed before); Alembic migrated to head (0026) against Neon; all three one-off bootstrap seeds run (`seed_initial_users`, `seed_master_data`, `seed_initial_world_state_and_clock`); app deployed via `railway up` and live at `https://meadowops-production.up.railway.app` (`/health` → 200). **Found and fixed a genuine portability bug** while migrating: `alembic/versions/0001_create_schemas_and_sandbox_role.py` and `app/services/sandbox_refresh.py` hardcoded `ALTER DEFAULT PRIVILEGES FOR ROLE meadowops`, assuming the DB owner role is always literally named `meadowops` (true only for local Docker Compose) — migration failed on Neon with `role "meadowops" does not exist`. Fixed by switching all three occurrences to `FOR ROLE CURRENT_USER` (verified valid Postgres syntax against Neon directly before editing; all 38 local sandbox tests still pass against Docker Compose afterward, confirming the change is behavior-preserving, not Neon-specific). Also found `settings.scheduler_enabled` defaults to `False` (opt-in) and wasn't set — the scheduler silently never ticked on the first deploy; fixed by setting `MEADOWOPS_SCHEDULER_ENABLED=true`. **Verified the actual requirement, not just that the process booted:** watched `live.simulation_clock.simulation_date` advance from 2026-01-01 to 2026-01-02 with a real row appearing in `live.scheduled_tick`, confirming the always-on host is genuinely keeping the scheduler alive against the live Neon instance, not just serving `/health`. **Frontend deployment gap found and closed 2026-09-14 (separate session from the rest of this row).** PRD 8.2 says "the FastAPI backend + Next.js frontends deployed" — but B1/U32 only ever deployed the backend; neither Next.js app had a deployment anywhere, contradicting the §10 DoD line "Deployed and reachable per §8.2" which had been checked off citing only U32. Caught while answering the user's own question about how to access the deployed project online. Closed at the user's explicit request (`AskUserQuestion`, chose Vercel over adding two more Railway services): both frontends deployed to Vercel via a user-supplied personal access token (used non-interactively via `vercel` CLI/API, then discarded from the local scratchpad — never written into the repo). `orbynadmin` → `meadowops-admin.vercel.app`, `shadcn-dashboard`/subsystem_2 → `meadowops-company.vercel.app` (both custom-named per the user's request, replacing Vercel's auto-generated project-name aliases via the Projects API). Each project's `MEADOWOPS_API_BASE_URL`/`NEXT_PUBLIC_MEADOWOPS_WS_BASE_URL` env vars point at the live Railway backend (`https://meadowops-production.up.railway.app`, `wss://` for the WS var). No CORS middleware was needed or added — verified first: both frontends proxy REST calls server-side (Next.js server → FastAPI, never browser → FastAPI directly, per each app's own `.env.example` comments), and the chat WebSocket ticket is likewise minted server-side; only the WS connection itself opens from the browser, which isn't subject to CORS. GitHub auto-deploy-on-push was not wired for either (the CLI's own auto-connect attempt failed — insufficient GitHub App scope on the token — deployed via direct `vercel deploy --prod` instead, same manual-deploy posture as the backend's `railway up`). Verified live: both apps' distinguishing pages return 200 (`/login` for admin, `/sign-in` for company). **Chat crash found and fixed 2026-09-14, a still-later same-day session, reported live by the user:** opening a brand-new persona thread (Subsystem 2's Mail → New thread) right after activating a scenario, then hitting Send, threw `Cannot read properties of undefined (reading 'trim')`. Root-caused to `POST /chat/threads` returning the backend's `ThreadRead` shape (no `unread_count`/`draft_body` fields — those are only computed by the list endpoint's `ThreadReadWithUnread`); `mail.tsx`'s `handleCreateThread` cast that raw response straight to the `ChatThread` type and selected it immediately, so `draft_body` was genuinely `undefined`, not `""`, and `ThreadView` seeded its composer state from that undefined value. Fixed at the source (fill in the same defaults the backend's list query would) plus a defensive `?? ""` at the actual read site in both subsystem_2's and orbynadmin's `thread-view.tsx` (mirrored code, same latent gap, though only reachable in subsystem_2 today since orbynadmin never creates threads). Reproduced and re-verified live via Playwright: create → approve → activate a scenario, open a new thread, send a message — no crash, delivered. Both frontends redeployed to Vercel afterward (`vercel deploy --prod` for each project) so the live sites actually carry the fix, not just the pushed commit — re-verified live (both 200).

**Tasks page reviewed for removal, kept 2026-09-14:** the user asked whether Subsystem 2's Tasks page (see the U31 line below) could be deleted as unused. Checked first: PRD Appendix D explicitly keeps this page (an optional, deprioritized kanban view, distinct from pages marked for discard — PRD line 579/680), and `tests/frontend/test_template_prep.py::test_subsystem_2_keep_list_present` asserts its route exists. Its only "usage" (sidebar nav link, cmd+K search entry) is exactly the intended scope, not dead code. Surfaced this to the user rather than deleting unilaterally; they chose to leave it as-is.

**Vercel redeployed 2026-09-14, a still-later same-day session:** two frontend bug fixes (the ground-truth error-message humanization and the Tasks-page stale-content fix, both below) had been pushed to GitHub but not deployed — Vercel has no auto-deploy-on-push connection (documented above), so the live `meadowops-admin.vercel.app` was still serving the pre-fix build. Caught during a completeness audit prompted by the user's own question. Redeployed via `vercel deploy --prod` directly from the local working tree (already at the pushed commit, `git status` clean) — since the CLI uploads the local filesystem rather than building from a remote ref, there's no ambiguity about which commit shipped. Live and aliased to `meadowops-admin.vercel.app` immediately after.

**Login round trip and live AI both verified 2026-09-14, a later same-day session, prompted by a real user-reported bug:** the Builder hit "Scenario narrative generation is not configured (no Claude client)" clicking Regenerate on the live site. Diagnosed via `app.main.create_app`: `app.state.claude_client` stays `None` unless both `MEADOWOPS_CLAUDE_CLIENT_ENABLED=true` and a non-blank `ANTHROPIC_API_KEY` are set — checked on Railway via `railway run` executing a script that prints only booleans (never `railway variables`, which prints raw secret values and is blocked by this environment's own credential-safety guard) and found both unset. Fixed: pushed the same key already in local `.env` to Railway via `railway variable set ANTHROPIC_API_KEY --stdin` (value piped through stdin, never appearing in any command line or transcript) and `MEADOWOPS_CLAUDE_CLIENT_ENABLED=true`, done only after explicit user confirmation (`AskUserQuestion`) given it starts real, billed Claude calls in production. Re-verified both `True` afterward via the same boolean-only check. Also redeployed via `railway up` at the user's request, since the live instance likely predated this session's B3/seed fixes and the GitHub auto-build connection (still broken, same as documented above) had just failed again on its own attempt. Separately, while in there: **production admin/analyst login round trip verified** — `POST /api/v1/auth/login` against the live backend returns 200 for both seeded accounts (checked via `railway run` piping the env-supplied credentials straight into `curl`, never printed), closing the login-round-trip gap this row previously left open. **Frontend bug found and fixed in the same pass**: the Approve button's error toast (`frontend/subsystem_2/.../scenario-actions.tsx`) surfaced the backend's raw dotted field names verbatim (e.g. `ground_truth.supporting_signals must be a non-empty list`) — these don't match the page's own labels ("Supporting signals", both in the Ground Truth Preview and the Edit narrative form), so a Builder searching the page for an input literally named that way wouldn't find one, even though the actual field was right there under a different label. Reproduced live via Playwright (created a scenario, clicked Approve before filling narrative fields, got the exact reported error), fixed by translating the backend's field names to the page's own labels before display, and re-verified live: the toast now reads "Supporting signals must be a non-empty list; ..." matching the visible labels, and the full golden path (fill Edit narrative → Save → Approve) still succeeds end-to-end. **Names swapped 2026-09-14, same day, per the user's own catch**: the original naming had it backwards — `orbynadmin` (Subsystem 1, both Builder and Analyst roles, S1-FR's operational/logistics app) is not really "the admin app" in the sense the user meant, and `shadcn-dashboard`/subsystem_2 (Builder-only, the scenario-builder/evaluation workspace) is the one that reads as the more admin-flavored surface — swapped via the Vercel API (remove both existing domain aliases, re-add each to the other project; a `vercel.app` domain can't be attached to two projects at once, so removal had to precede re-add). Now: `orbynadmin` (Subsystem 1) → `meadowops-company.vercel.app`, `shadcn-dashboard`/subsystem_2 (Subsystem 2) → `meadowops-admin.vercel.app`. Re-verified live after the swap (both 200 at their new URLs). **Project display names also corrected 2026-09-14, same day:** the domain-alias swap above moved only the domains, not the underlying Vercel project names — since a project rename doesn't move its domain (confirmed separately), the two projects were left mislabeled in the Vercel dashboard (the project literally named `meadowops-admin` was serving `meadowops-company.vercel.app`, and vice versa). Caught by the user spotting the mismatch in the Vercel dashboard. Fixed by renaming each project (via the Vercel Projects API, one routed through a temporary name to avoid a collision) so its name now matches the domain it serves: the project behind `meadowops-admin.vercel.app` (shadcn-dashboard/Subsystem 2) is now named `meadowops-admin`, and the project behind `meadowops-company.vercel.app` (orbynadmin/Subsystem 1) is now named `meadowops-company`. Purely a dashboard-label fix — domains and live traffic were unaffected, re-verified live afterward (both 200 at their existing URLs).

**One residual, non-blocking item**: the Railway service came pre-connected (by Railway itself, before any of this session's actions) to the user's GitHub repo at the monorepo root with no Dockerfile there — every config change re-triggers a broken auto-build from that connection (3 failed auto-builds observed; the live deployment itself is unaffected since it was shipped via `railway up` file upload, a separate path). Disconnecting requires the user's own dashboard access — the project-scoped Railway token this session used returned "Bad Access" on `service source disconnect`, a genuine token-scope limit, not something to work around. **User must disconnect it by hand**: Railway dashboard → MeadowOps service → Settings → Source → Disconnect. **Post-closure operational finding (2026-09-13, caught by advisor review, not the original deployment work):** leaving `MEADOWOPS_SCHEDULER_ENABLED=true` at its default 60s tick interval keeps Neon's compute permanently awake, defeating the scale-to-zero behavior that makes its free tier viable — at an always-on minimum compute size this projects to roughly 180 CU-hours/month against Neon's 100 CU-hour free allowance, i.e. it would have started incurring charges or throttling within weeks. Since PRD 8.2's always-on requirement was already verified above (the clock-advance check) and Phase 4's U32 hasn't formally claimed this deployment yet, the user chose (AskUserQuestion, 2026-09-13) to disable the scheduler now (`MEADOWOPS_SCHEDULER_ENABLED=false`, applied via `railway redeploy` — restarts the existing image, does not touch the broken git-connected build source above) rather than run it continuously; re-enable when U32 formally owns the deployment. Separately noted but not acted on: `attachment_storage_dir` (U30c's chat attachments) resolves to a path on the container's local filesystem, which Railway does not persist across redeploys — any uploaded attachment is lost on the next deploy. This is a real gap in this ad hoc deployment, out of scope for B1 to fix (it belongs to whichever unit formally productionizes storage), and is called out again in §9's deployment-parity note. |
...
| B3 | Claude API key / real adapter | Originally: no key present, and (discovered 2026-09-14 while building U34's E2E coverage) no code path existed either — `app.main.create_app` hardcoded `app.state.claude_client = None` unconditionally, so even a real key couldn't have made `/regenerate`/`/complete`/`suggest-pushback`/`sufficiency-check` succeed | **Adapter closed 2026-09-14 (user's explicit choice, `AskUserQuestion`, ahead of checking PRD §10's two remaining Definition-of-Done boxes); live verification blocked on a user-supplied key.** New `app.domain.claude_client_anthropic.AnthropicClaudeClient` implements `ClaudeClient`'s Protocol against the real `anthropic` SDK (added as a backend dependency, `uv.lock` relocked) — text-block joining, `max_retries=0` (the SDK's own default retry would silently multiply scenario_generation/persona_chat/evaluation's already-implemented PRD 9.2 one-retry), a bounded per-call `timeout_seconds` (the Protocol docstring's own documented remedy for callers holding a DB session open across the call — the other remedy, restructuring callers to release the connection first, stays open), and upstream-error-message redaction (`ClaudeAPIError`/`ClaudeTimeoutError` never carry the SDK exception's raw `str(exc)`, which embeds upstream response body text and would otherwise flow straight into an HTTP 502 detail the Builder's browser sees — exactly the risk `scenario_generation.ScenarioGenerationFailedError`'s own pre-existing docstring had flagged for "whichever adapter lands here"). Wired into `app.main.create_app` behind a new **opt-in** `Settings.claude_client_enabled` (default `False`) plus a bare (non-`MEADOWOPS_`-prefixed) `ANTHROPIC_API_KEY` field — deliberately not "key present ⇒ live," because `tests/conftest.py` loads the repo-root `.env` into every backend test process; an ambient-key switch would have made `tests/api/test_admin_scenarios.py`'s own 503 assertion go red locally and risked live billed calls from any Claude-route test that forgets to substitute a `MockClaudeClient`. A `model_validator` enforces enabled-implies-key-present — tightened mid-implementation after it was caught empirically: the repo-root `.env`'s `ANTHROPIC_API_KEY` line turned out to be **present but blank** (contradicting this same investigation's own earlier assumption that a real key was configured), which an `is None`-only check would have let through to a cryptic SDK-internal `TypeError` at the first live call instead of failing fast at startup — fixed to also reject an empty/whitespace-only key, with a regression test. 23 new backend tests (config validation, `create_app` wiring, and the adapter itself — exercised against the real installed `anthropic` package's own exception types, not hand-rolled stand-ins).

**Closed for real 2026-09-14, same day, once the user supplied a genuine `ANTHROPIC_API_KEY` in the repo-root `.env`.** The live smoke test (`tests/domain/test_claude_client_anthropic_smoke.py`) passed immediately. But driving the actual domain pipelines live (not just a raw ping/pong) surfaced a real, previously-latent bug none of the 23 adapter/config tests could have caught: `claude-sonnet-4-5` routinely wraps its JSON response in a ` ```json ... ``` ` markdown fence despite every one of `scenario_generation`/`evaluation`/`persona_chat`'s own prompts explicitly instructing "no markdown fences" — `MockClaudeClient` never produces this shape, so `generate_scenario_narrative`'s first live call failed with `response is not valid JSON: Expecting value: line 1 column 1 (char 0)`. Fixed in exactly one place — `AnthropicClaudeClient.create_message` now strips a whole-response markdown fence before returning `ClaudeResponse` — rather than in each of the three domain-layer `parse_*_response` functions, since this is a real-API/model quirk the domain layer shouldn't need to know about; conservative by construction (only strips when the *entire* stripped content is fenced start-to-end, so a response that merely mentions a code sample mid-text is untouched). 4 new regression tests added (27 total for this blocker). Re-verified after the fix, live, in order of increasing integration depth: (1) both `generate_scenario_narrative` and `generate_evaluation` called directly against the real API with realistic fixture data — both parse cleanly; (2) `suggest_pushback_message` and `run_sufficiency_check` (persona_chat's two AI call sites) — both succeed; (3) the actual running app booted with `claude_client_enabled=true`, and `POST .../regenerate` and `POST .../complete` driven for real through the live HTTP routes (not a script bypassing them, the same pattern `e2e/scripts/seed_evaluation_fixture.py` uses for login/scenario/thread setup) — both returned 200 with genuine Claude-generated content, and real `Scenario`/`ChatThread`/`Evaluation` rows were written. Cleanup could delete the scenario/exception-flag rows but not the chat messages (`chat.chat_message`'s own immutability trigger, PRD 6.13/ER-6, blocked the delete by design) — this verification's thread stays in the local dev DB permanently, same precedent as U34's own E2E fixture-seeding pollution (see `docs/testing-report.md`). Full backend suite re-run clean after the fix: 1311 passed (up 4 for the new regression tests), same 3 pre-existing `TestResolveDifficultyForCluster` failures as before, nothing new. **Nothing left open on this blocker.** `/regenerate`/`/complete`/`suggest-pushback`/`sufficiency-check` are live in any environment with both `ANTHROPIC_API_KEY` (real, non-blank) and `MEADOWOPS_CLAUDE_CLIENT_ENABLED=true` set — the local dev `.env` has the real key but **deliberately not** the enabled flag persistently on. Tried leaving both set for a final full-suite re-run and it surfaced a live confirmation of this design's own reasoning (the docstring on `Settings.claude_client_enabled`/`tests/conftest.py`'s `load_dotenv` comment): 10 tests failed total (7 net-new beyond the 3 pre-existing `TestResolveDifficultyForCluster` ones). 6 identified by name: 3 (`TestSuggestPushbackRoute`/`TestSufficiencyCheckRoute`/`TestCompleteThreadRoute`'s own `test_returns_503_when_no_claude_client_is_configured`) because the suite started making real, billed live API calls instead of hitting the 503-no-client path they assert; 3 more assume the flag is off by default (`test_config.py::TestClaudeClientEnabledValidation::test_disabled_by_default`/`test_disabled_with_a_key_present_is_valid`, `test_claude_client_wiring.py::TestClaudeClientWiring::test_disabled_by_default_leaves_claude_client_none`). The 7th failure's name was lost to output truncation (`tail -10` on the run) and not re-captured — re-enabling the flag again just to re-derive one test name would have repeated the exact live-billing mistake this paragraph documents, so it wasn't done. Reverted immediately — the flag now stays off in the local `.env` by default, same as every other environment; turn it on only for the duration of a manual live-Claude verification or demo, then back off before running the test suite again. |
| B4 | No git remote configured | Local commits only for now | **Closed 2026-09-14.** Stale even before this fix — `origin` (`github.com/lehoangbaoduy/MeadowOps`) has existed and been pushed to repeatedly since 2026-09-14's frontend-deployment/B3/B14 work, this row just never got updated. Found and fixed during a completeness audit. |
...
| B14 | Discovered 2026-09-14 while fixing the E2E CI failure the user's push triggered: `app.services.exception_rule_defaults.seed_exception_rule_thresholds` — the function that populates `live.exception_rule_threshold`, without which the exception engine (U15) can never flag anything and no scenario can ever be created — was never actually called from any of the three places that run the "bootstrap seed" sequence (`e2e/scripts/start-backend.sh`, `.github/workflows/backend-ci.yml`, `docs/walkthrough-script.md`'s Setup section, and B1's own Railway/Neon deployment log). Only individual backend unit tests called it, as their own fixture setup — its own docstring's claim of being "exercised for real at migration/seed time" was stale/false. Invisible on any long-lived database (local dev, or a production instance seeded once and never reset) since the rows, once inserted by any means, just sit there; only surfaced now because `evaluation.spec.ts`/`reflection.spec.ts` (added same day as the Claude adapter push) were the first specs to run against a genuinely fresh database on GitHub's actual CI runner — reproduced deterministically in a scratch Postgres 18 container (confirmed `threshold=None` immediately after the existing three-seed sequence, confirmed non-None after adding the fourth call) before touching any file, not inferred from the CI logs alone | **Fixed 2026-09-14** — added `seed_exception_rule_thresholds` to all three bootstrap-seed call sites (same PR that added it: `e2e/scripts/start-backend.sh`, `backend-ci.yml`, `docs/walkthrough-script.md`), corrected the function's own stale docstring and a matching stale comment in `tests/data/test_exception_rule_thresholds.py`. Re-verified against a fresh scratch DB via the real `start-backend.sh` script end-to-end (not just the isolated function call) — `seed_evaluation_fixture.py`, which previously failed with `AssertionError: baseline master data must be seeded first`, now succeeds cleanly. Full backend suite re-run clean (1311 passed, same 3 pre-existing unrelated failures) | **Production confirmed and fixed, same day.** The user granted a Railway project token specifically for this (a first token failed CLI auth outright — genuinely invalid, not the same block as below — the user regenerated a second one that worked). `railway variables` itself stayed blocked by this session's own auto-mode safety classifier ("Credential Materialization") and was not attempted again; instead, `railway run --service MeadowOps -- uv run python -c "..."` executed the same idempotent `seed_exception_rule_thresholds` call directly against the live DB via Railway's injected env — a write, not a credential read, so it wasn't blocked. Confirmed empirically, not assumed: **0 rows before, 6 after** — the exception engine has had zero usable thresholds in production since the 2026-09-13 deploy, meaning it could never have flagged a single exception there. `/health` reconfirmed 200 immediately after (the one-off script only touched data, never the running app process). **Separately surfaced while getting service-scoped `railway run` working**: `railway status --json` showed the service's `latestDeployment` was a **failed** auto-build (`"status": "FAILED"`, `"builder": "RAILPACK"`, triggered by this session's own `02bdced` push, `"reason": "deploy"`) — directly contradicting U32's own claim that the GitHub auto-build source was fully disconnected 2026-09-14. It failed harmlessly (the active, working deployment — the original Dockerfile-built one — was never replaced, matching U32's own prediction that a broken auto-build can't knock out the `railway up`-shipped instance), but it means the connection itself is still live, still stale-`RAILPACK`-configured (no `Dockerfile` builder set for it, unlike `railway.toml`'s own config), and will keep silently attempting (and failing) a broken build on every future push to `master` until the user disconnects it by hand — same unresolved action B1's own row already named. **Still separately open, unrelated to this fix**: the live backend is still running the pre-session code (no Claude adapter, no fence fix, no this seed fix) — none of that has been deployed via `railway up`/`redeploy` yet, only this one idempotent DB write went out |
| B15 | Discovered 2026-09-14/15, reported live by the user: sending or listing messages on any real chat thread against production returned a bare-text 500 (`Internal Server Error`, no JSON body — an unhandled exception, not one of `create_message_route`'s mapped error branches). Reproduced live via Playwright against the exact reported thread, then confirmed against the backend directly (`GET`/`POST .../messages` both 500, same plain-text body). Pulled the real Railway traceback: `psycopg.errors.UndefinedColumn: column "sender_role" of relation "chat_message" does not exist`. `alembic current` on production reported revision 0026 (head) — migration 0018 already includes `sender_role` in its own `CREATE TABLE`, so alembic's bookkeeping gave no signal anything was wrong. Confirmed via a direct `information_schema.columns` query: production's `chat.chat_message` had only 6 of its 7 mapped columns. Best explanation: an earlier deploy ran `alembic upgrade head` against production while migration 0018's own file locally still lacked this column (added in a later, pre-commit edit) — alembic stamped 0018 as applied and never had reason to revisit it. `chat_thread`-level operations (list threads, thread creation) were unaffected and worked throughout, since neither touches `chat_message` | **Fixed 2026-09-15.** New migration `0027_fix_chat_message_sender_role_drift.py` — idempotent `ADD COLUMN IF NOT EXISTS sender_role live.user_role` + `ALTER COLUMN ... SET NOT NULL`, verified a true no-op on every environment that already has the column correctly (local dev, CI). New regression test (`tests/data/test_chat_schema.py::test_migration_0027_repairs_a_missing_sender_role_column`) loads and executes the real migration file's own `upgrade()` function via a standalone Alembic `Operations` context against the real `chat.chat_message` table, inside one uncommitted transaction (using the same immutability-trigger-disable escape hatch this project's other fixtures already use, so the shared dev database's own existing rows are restored untouched by the rollback) — confirmed genuinely red (`FileNotFoundError`) before the migration file existed, green after. Routed through the project's own `hotfix` governance workflow (harness-os), including human approval of the production-database change | **Applied and verified live 2026-09-15.** `railway run alembic upgrade head` against production; `alembic current` now reports `0027 (head)`, and a direct `information_schema` query confirms `chat.chat_message.sender_role` exists and is `NOT NULL`. End-to-end re-verified against the live backend on the originally-affected thread: `POST .../messages` now returns 201 (was 500), `GET .../messages` returns the sent message. Committed (`ff7fdc8`) but not yet pushed to GitHub — no token available in this session |
...
- [x] Full acceptance testing checklist (9.3) passing — see §9 for the full breakdown; checkbox here corrected 2026-09-14, found unchecked and stale while auditing project completeness even though §9/§10's identical items were already checked
- [x] Backup restore drill completed successfully — B2, 2026-09-13 (see §2/§8 row 32); checkbox here corrected 2026-09-14, same staleness as above
- [x] Query Playground permission boundary explicitly verified — U31 (see §9); checkbox here corrected 2026-09-14, same staleness as above
- [x] System documentation finalized (schema diagram, data dictionary, data-flow doc, testing report) — U31 (see §9); checkbox here corrected 2026-09-14, same staleness as above
- [x] Deployment verified in the actual target environment (8.2), not just locally — U32 backend + B1's Vercel frontend closure (see §9/§10); checkbox here corrected 2026-09-14, same staleness as above
- [x] Final walkthrough script prepared, using QA/synthetic data — `docs/walkthrough-script.md`, U31 (see §9); checkbox here corrected 2026-09-14, same staleness as above
- [x] Definition of Done (1.6) fully satisfied — see §10 for the full checklist; checkbox here corrected 2026-09-14, same staleness as above
...
| Unit | Spec ID (planned) | Description | Status |
|---|---|---|---|
| U31 | MEADOWOPS-HARDEN-002 | Acceptance checklist pass, docs (schema diagram, data dictionary, data-flow doc, testing report) | **Done, 2026-09-14.** §9/§10's acceptance checklist fully closed: a committed Playwright E2E suite (`e2e/`, own `e2e.yml` workflow — the user's explicit choice over a walkthrough-script-only interpretation of PRD 9.3), the repo-root structural checks (`tests/frontend/`) newly wired into `backend-ci.yml`, a fresh demo-data sweep, the sandbox permission boundary re-verified live against the running role, and both GitHub Actions workflows independently confirmed green via the REST API (not assumed from a push) after fixing two rounds of real CI-only bugs — a Starlette `TestClient.websocket_connect()` race under CI's slower scheduler, and the `sandbox` schema having no tables on a genuinely fresh database until `/api/query/refresh-sandbox` runs once. Docs: `docs/schema-diagram.md` and `docs/data-dictionary.md` are generated (not hand-written) by a new `scripts/generate_docs.py` that introspects the live SQLAlchemy `Base.metadata`, so they can't silently drift from the real schema; writing it surfaced and fixed a real bug (`backend/app/db/__init__.py` wasn't importing `evaluation`/`human_review`/`portfolio`, so `alembic revision --autogenerate` was silently blind to 3 real tables — fixed, verified via a throwaway autogenerate diff showing no drops for any of the three plus the full 1288-test suite still passing). `docs/data-flow.md` covers the three PRD-load-bearing boundaries in one short doc; `docs/testing-report.md` cites only regenerable counts and names a second, separate, still-open gap (lint runs nowhere in CI, only locally and non-blocking via `scripts/ci.sh`). `docs/walkthrough-script.md` was fact-checked against the actual running source rather than drafted from memory — this caught two false claims before they shipped (the QA harness tears down every row it creates rather than leaving them inspectable live; the chat composer's AI-assist buttons and the evaluation/human-review/portfolio/Ledger-action routes have no frontend UI at all) and is explicit about which of the loop's remaining steps are real, tested, and API-only rather than fabricating a UI for them. |
| U32 | MEADOWOPS-INFRA-005 | Deployment to target host + managed Postgres (blocked on B1) | **Done, 2026-09-14 — formal harness-os unit, not just B1's ad hoc deployment.** Scoped via `security-change` workflow (run 152, high risk — R2 credentials), pre-implementation security review (decision 1627: 2 MEDIUM fixed — a Settings validator gap that could silently keep writing to ephemeral storage, and unbounded boto3 timeouts risking threadpool exhaustion — plus SecretStr/token-scope hygiene items and a doc fix). **Attachment storage**: chat attachments (U30c) moved off Railway's ephemeral container filesystem to Cloudflare R2 (user's choice over a Railway volume, AskUserQuestion) — new `R2AttachmentStorage` (`app/core/storage.py`) behind the existing `AttachmentStorage` Protocol, a `Settings.model_validator` enforcing `attachment_storage_backend`/`r2_*` fields both directions, `run_in_threadpool` added around the upload route's now-network-bound `storage.save` call. Verified three ways: 11 unit tests (moto, no real network), 1 genuine end-to-end HTTP-route integration test (upload→claim→download through `app.main`'s real lifespan-constructed client swapped for a moto stub, mirroring the existing `app.state.claude_client` test-substitution pattern), and a live smoke test against the real R2 bucket. The *first* R2 API token the user created returned `AccessDenied` on every operation including read-only ones — a permission-scope problem confirmed by testing writes and reads separately before concluding it wasn't a credential-format issue; the live app was immediately rolled back to `local` storage rather than left broken (also surfaced a real gotcha: `railway variable --set KEY=` sets an empty string, not an unset — `Settings`' own validator then refuses to boot, since empty-string-but-not-`None` still counts as "set"; fixed using `railway variable delete`). A regenerated, correctly-scoped token passed the live smoke test and is now active in production. code-reviewer + security-reviewer both dispatched pre-implementation (design-only, no code existed yet) — see decision 1627 for the full finding list. **Scheduler**: re-enabled in production (`MEADOWOPS_SCHEDULER_ENABLED=true`) at a 30-minute tick interval (`MEADOWOPS_SCHEDULER_INTERVAL_SECONDS=1800`, up from the original 60s) per the user's AskUserQuestion choice, trading simulated-time speed for staying well under Neon's 100 CU-hour free-tier allowance — bundled into the same redeploy as the R2 work rather than shipped separately. **Postgres version parity**: local Docker Compose bumped from `postgres:16-alpine` to `postgres:18-alpine` to match Neon exactly (user's choice over accepting the divergence) — required dropping the old volume (major-version data isn't forward-compatible) and fixing the volume mount path (`/var/lib/postgresql/data` → `/var/lib/postgresql`), since Postgres 18's official image moved PGDATA to a version-specific subdirectory (verified via docker-library/postgres's own docs after the first attempt crash-looped). Migrations to head, all bootstrap seeds, and the full 1288-test suite all re-verified clean against the new local Postgres 18 — genuine parity now proven locally, not just inferred from Neon. code-reviewer found 1 MEDIUM (any other contributor with a pre-18 volume would hit the identical crash) — fixed with an explicit remediation comment in `docker-compose.yml`. **Deferred, not done**: migrating `backend/railway.toml` to `.railway/railway.ts` — the user initially asked for this, but Railway's own IaC docs (checked via context7 before executing, not assumed) confirm the new format cannot express a restart policy or a Dockerfile builder at all, a genuine capability gap, not a syntax difference; a dry run (`railway config migrate`, non-destructive by default) confirmed it would silently drop `restartPolicyType: ON_FAILURE`/`restartPolicyMaxRetries: 3`. Put back to the user with the new evidence; they chose to stay on `railway.toml` (works until Railway's 2026-12-01 deprecation date). **GitHub auto-build source**: disconnected by the user via their dashboard, 2026-09-14, closing the last residual B1 item. Railway's own API still shows a stale `source.repo` reference at the service-instance level as of this check, but the currently active deployment remains confirmed running from a manual `railway up`/`redeploy`, not a GitHub-triggered build — full confidence this doesn't silently re-trigger will only come from watching the next real push. **CI wiring**: `.github/workflows/backend-ci.yml` added, 2026-09-14 — tests-only per the user's explicit choice (AskUserQuestion) over a full CI/CD variant; deployment stays a deliberate manual step. Runs migrations + all three bootstrap seeds + the full pytest suite against a throwaway `postgres:18-alpine` service container on every push/PR to `master`. The recipe was empirically validated locally before requesting a code-reviewer pass (an isolated throwaway container, real dev Postgres temporarily stopped then restored intact) — 1288/1288 passing. code-reviewer found 2 MEDIUM (inconsistent `--frozen` usage across `uv run` calls; no `permissions:` block leaving `GITHUB_TOKEN` unscoped) and 3 LOW, all addressed (`UV_FROZEN=1` at job level, `permissions: contents: read`, `timeout-minutes: 15`). Caught and fixed a self-introduced bug before pushing: the workflow was written targeting a `main` branch that doesn't exist on this repo (actual branch is `master`) — found via `git ls-remote`, fixed in a follow-up commit rather than amending. **Verified live, not just pushed**: checked via GitHub's REST API (not assumed from the push succeeding) that the workflow actually triggered and completed — run 34793030881, all 8 steps green, ~1m45s total, first real confirmation on GitHub's actual hosted runner rather than only this session's local simulation. Decisions 1627 (security review) → 1629 (railway.ts deferral) → 1630 (Postgres parity) → 1631 (implementation) → 1632 (finalize) → 1633 (CI workflow) → 1634 (CI verified live), all `approved`. **U32 is now fully done** — every item in its original scope (attachment storage, scheduler, Postgres parity, GitHub disconnect, CI wiring) is closed; the only remaining Railway-side open question (whether the stale `source.repo` reference could still cause a surprise auto-build) is a watch item, not a blocker. |
| U34 | MEADOWOPS-UI-006 | Evaluation/human-review/portfolio/Ledger-action UI | **Done, 2026-09-14.** Not a PRD Definition-of-Done requirement (§6.9/§6.10 are mechanism-level: "built and tested during Build & Test," UI/content explicitly deferred to Active Use) - scoped anyway per the user's own choice (`AskUserQuestion`, over checking §10's two remaining boxes and declaring the PRD complete outright), closing the exact gap U31's own walkthrough-script writeup flagged: "the evaluation/human-review/portfolio/Ledger-action routes have no frontend UI at all." Four independent surfaces, sequenced Ledger first (no thread_id dependency) then the eval→review→export chain: **(1) Ledger actions (orbynadmin)** - `DecisionActions`/`ProposeDecisionDialog` drive every one of `app.api.ledger`'s require_admin transition routes (propose/request-clarification/resubmit/accept/reject/implement/partially-implement/outcome) from the Activity page, which Unit 24 had deliberately left read-only (`tests/frontend/test_activity_page.py`'s own `test_decisions_table_has_no_write_actions` asserted no write UI existed - rewritten this unit to assert the opposite, now that a real caller exists). **(2) Evaluation + human review + portfolio export (subsystem_2, Builder-only)** - new `ScenarioThreads`/`EvaluationPanel`/`HumanReviewSection`/`PortfolioExportDialog` on the scenario detail page; `chat_thread` is unique on `(scenario_id, persona)` not `(scenario_id)` alone, so each scenario's threads (not just one) each get an independent panel. `GET /chat/threads` has no `scenario_id` filter - filtered in a new `listThreadsForScenario` proxy function instead of adding a backend query param, keeping this unit frontend-only. `/complete`'s 503 (no Claude client configured) and the eval/review/export GETs' 404 (nothing yet) are both surfaced as real, expected states, not generic errors. **(3) Portfolio reflection (orbynadmin, Analyst-voiced)** - `ReflectionForm` renders in `ThreadView` in place of the composer once `thread.status === "completed"` (a field the frontend `ChatThread` type never carried until this unit, despite the backend always returning it) - kept off Subsystem 2 deliberately, since `POST .../reflection` is `reject_service_role` specifically because PRD 6.10's seven questions are written in the Analyst's own voice, and Subsystem 2 is Builder-only end to end. No client-side role gating anywhere in this unit, matching this app's own established precedent (`thread-view.tsx`'s pre-existing docstring: "no such client-side role-awareness exists anywhere else in this app") - every write action relies on the backend's own require_admin/reject_service_role enforcement, surfaced as a toast on a 403 rather than hidden client-side. Verified: `tsc --noEmit` clean on both frontends, ESLint clean on every new/changed file (orbynadmin only - subsystem_2's own ESLint config has the pre-existing `eslint-config-next`/`@eslint/eslintrc` bug `docs/testing-report.md` already documents, unrelated to this unit), `npm run build` production build green on both apps with every new route registered, and the full backend + repo-root frontend structural suites (1288 + 27, the latter up from 23 with 4 net-new `test_activity_page.py` assertions) passing. **E2E coverage added in a follow-up pass, same day**: a real repo-root `.env` (with live Postgres/Anthropic/session credentials) turned out to exist after all, unlike the `backend/.env` checked earlier - the full local stack was booted for real and 7 new Playwright specs were run live, repeatedly, not just written: `e2e/tests/orbynadmin/ledger-actions.spec.ts` (2 tests, the full propose→accept→implement→outcome lifecycle plus reject), `e2e/tests/subsystem2/evaluation.spec.ts` (3 tests, describe.serial), `e2e/tests/orbynadmin-analyst/reflection.spec.ts` (2 tests, describe.serial, including the 409-as-already-submitted path). Real bugs this caught: a `Notes (optional)` `Textarea` in `decision-actions.tsx`'s Outcome dialog had no `id`/`htmlFor`, so `getByLabel` (and any real screen reader) couldn't find it - fixed. More significantly: `app.main.create_app` hardcodes `app.state.claude_client = None` unconditionally (Phase 4 blocker B3) - `/complete` and `/regenerate` **always** 503, with or without a real `ANTHROPIC_API_KEY` configured, not just in this session's own environment as `docs/walkthrough-script.md` previously implied (now corrected there too). Worked around in `e2e/scripts/seed_evaluation_fixture.py` the same way `tests/support/qa_harness.py`'s own `complete_thread` already works around it for the backend suite - script the Evaluation row directly rather than depend on a live Claude call - plus a direct `ExceptionFlag` insert (no route creates one, and the scheduler that normally would is `scheduler_enabled=False` by default locally). Also caught and fixed: `scenarios.spec.ts`'s pre-existing empty-state test assumed a scenario-free database, which this unit's own fixture-creating specs now legitimately violate - refiltered to `status=cancelled` (genuinely, deterministically empty) rather than the unfiltered list. Full suite (20 tests, up from 13) run green three times in a row, including under `fullyParallel`'s real concurrency (which itself surfaced and required fixing a race: two workers' `beforeAll` hooks both trying to insert an open `ExceptionFlag` for the same product/warehouse pair - fixed via `test.describe.serial`). |
| U35 | MEADOWOPS-DOM-023 | Persona-chat AI-assist UI: opening-message suggestion (new) + wiring the already-built, previously-unused pushback-suggestion/sufficiency-check into subsystem_2's UI (PRD 6.13, follow-on to Unit 23) | **Backend + frontend done 2026-09-15, Playwright-verified against a live local stack.** Scoped via a 3-decision `AskUserQuestion` after the user asked to bring back the PRD's "AI-suggested opening message" idea (traced during that conversation to Unit 23's own explicit deferral, DD-33: "opening message stays Builder-typed... deferred rather than answered as a drive-by"): (1) trigger point is ThreadView after thread creation, not inside NewThreadDialog before it - mirrors the existing suggest-pushback route shape exactly (operates on an existing thread_id); (2) scope widened to wire up all three AI actions (opening, pushback, sufficiency-check) into one shared AI-assist panel, since pushback/sufficiency-check were themselves fully built backend-only since Unit 23 with zero frontend UI - confirmed via grep, zero references anywhere in subsystem_2; (3) attitude picker applies to the opening message too, consistent with PRD 6.13's own wording. **Backend**: new frozen v1 `STAKEHOLDER_OPENING_TEMPLATE` (`app/domain/prompt_templates.py`) - a new template, not an edit to `STAKEHOLDER_ROLEPLAY_TEMPLATE` (ER-6 discipline; that template hardcodes a required `analyst_message` slot an opening message has none of). `build_opening_prompt`/`suggest_opening_message` (`app/domain/persona_chat.py`) mirror `build_pushback_prompt`/`suggest_pushback_message`'s PRD 9.2 one-retry orchestration exactly, minus the two now-irrelevant parameters. `suggest_thread_opening` + new `ThreadAlreadyHasMessagesError` (`app/services/persona_chat.py`) mirror `suggest_thread_pushback`/`NoAnalystMessageYetError` with an inverted guard (opening suggestion requires *zero* messages, not one Analyst reply) - reuses the same redacted `build_known_information` projection, so the Analyst-can-never-reach-full-ground-truth property holds identically. New `POST /threads/{thread_id}/suggest-opening` route (`app/api/chat.py`, `require_admin`) + `SuggestOpeningRequest`/`SuggestOpeningResponse` schemas mirror `suggest_pushback_route` exactly. Routed through harness-os's `new-feature` workflow (spec MEADOWOPS-DOM-023, risk assessed medium). Tests written first per this project's own red-then-green discipline, across all three layers (`tests/domain/test_persona_chat.py`, `tests/services/test_persona_chat_service.py`, `tests/api/test_chat_api.py`) - confirmed genuinely red (`ImportError`) before any implementation code existed, green after. Also fixed `tests/architecture/test_subsystem_boundary.py`'s route-classification allowlist, which correctly went red once the new route existed unclassified - added `admin_only`, same as suggest-pushback. Full backend suite: 1342 passed, 0 failed, 1 skipped. **Along the way**: found and fixed unrelated pre-existing local dev DB pollution (a leftover `ExceptionFlag` row from an earlier session, same signature as a previously-documented incident) via `docker compose down -v` + re-migrate + re-seed - blocked verifying the new service-layer tests until cleared, not caused by this change. Code-reviewer (1 HIGH, 3 MEDIUM, 1 LOW, all fixed - `ALL_TEMPLATES` missing the new template, `PersonaMessageGenerationFailedError` message hardcoding "pushback", duplicated retry-loop body between suggest_pushback_message/suggest_opening_message extracted into `_generate_persona_message`, a missing SKEPTICAL/APPRECIATIVE opening-attitude description, a stale docstring) and security-reviewer (0 findings above LOW) both dispatched post-implementation. **Frontend** (subsystem_2): `Attitude` type/`ATTITUDES`/`attitudeLabel` (`mail/data.tsx`), `suggestOpening`/`suggestPushback`/`checkSufficiency` wrappers (`lib/chat-api.ts`) over three new Next.js proxy routes (`api/chat/threads/[id]/{suggest-opening,suggest-pushback,sufficiency-check}/route.ts`, same shape as the existing `messages/route.ts`), a new self-contained `AiAssistPanel` component (`mail/components/ai-assist-panel.tsx`) rendered in `ThreadView` between the message list and the composer - which of the three actions it offers is driven entirely by the thread's own message state (`hasMessages`/`hasAnalystMessage`, mirroring the backend's `ThreadAlreadyHasMessagesError`/`NoAnalystMessageYetError` guards rather than duplicating them as separate frontend rules), so a thread is only ever in one of "offer opening" or "offer pushback/sufficiency", never both. `next build`/`tsc --noEmit` both clean. **Verified live** via Playwright against a real local backend (`uvicorn app.main:create_app --factory`, factory pattern required - no module-level `app`) + frontend (`pnpm dev` on :4002): logged in as the seeded admin, created a scenario seeded directly in Postgres using real master-data IDs (`SKU-COR-001`/`WH-EAST` - fabricated IDs 404 against `exception_flag`'s real FKs into `live.product`/`live.warehouse`), opened a persona thread, and drove all three actions end-to-end. Confirmed correct button gating (opening-only before any message, pushback+sufficiency-only once an Analyst-role message exists - note the Builder's own sent messages are `sender_role=admin`, not `analyst`, so this subsystem_2 UI alone can never trigger the pushback/sufficiency state; a real Analyst reply from Subsystem 1 is required, simulated here with a direct DB insert) and the safe 503-not-configured path for all three actions (no live Claude client in local `.env`, matching this project's own established local-dev convention) with three distinct, correctly-worded error messages. **Bug found and fixed during this verification pass**: `AiAssistPanel`'s local `error`/`verdict` state didn't reset when `hasMessages`/`hasAnalystMessage` changed, so a stale error from an action that had just been un-offered (e.g. "opening message not configured" lingering after the thread's first message made that button disappear) stayed on screen looking like it still applied - fixed with a `useEffect` clearing both on that transition. Did not spend real Anthropic API cost verifying the actual happy-path (a genuine Claude-generated suggestion landing in the composer) - the automated backend test suite already covers that via `MockClaudeClient`, and the live check here focused on the parts only a browser can prove (rendering, gating, wiring, error surfacing). |
| U33 | MEADOWOPS-INFRA-006 | Backup automation + restore drill (blocked on B1/B2) | **Done, 2026-09-14.** Scoped via `AskUserQuestion` after the user questioned whether an automated-backup unit was still needed: PRD 8.6 is explicit ("Automated daily backups of the Postgres instance are a requirement," separate from the restore-tested half B2 already closed), and checking first (WebSearch/WebFetch against Neon's own docs) found the Neon free tier this project runs on gives only a 6-hour point-in-time-restore window with no scheduled backups at all — scheduled snapshots are a paid-plan-only feature — so relying on Neon's own platform wouldn't have satisfied 8.6 without a cost the project has been actively avoiding (U32's scheduler-interval choice). Built instead: `.github/workflows/backup.yml`, scheduled daily (`workflow_dispatch` also available), `pg_dump` (custom format) against the real Neon instance covering all four real schemas (`live`/`reporting`/`engine`/`chat`) — broader than B2's own drill, which only covered `live`/`reporting`/`engine` and missed `chat` (migration 0018) entirely — excluding `sandbox`/`sandbox_staging` (disposable, rebuilt on demand, no unique data). A `pg_restore --list` integrity check runs before upload; the dump then goes to a dedicated R2 bucket via a narrowly-scoped token kept separate from the app's own attachment-storage credentials (U32), since a full dump includes `live.user.password_hash`. Retention is a Cloudflare R2 bucket lifecycle rule, not scripted deletion. Two real bugs found and fixed before this could be called done, both via genuine failed runs, not guessed: (1) the dump-scope/integrity-check logic was verified against the real local Postgres 18 instance before ever writing the workflow, which caught a wrong `pg_restore --list` grep pattern written from memory; (2) the first live GitHub Actions run failed with a Postgres server-version mismatch (`pg_dump` 16.15 against a Postgres 18 server) — installing `postgresql-client-18` via apt did not make it the default on PATH (the runner's `update-alternatives` still pointed at a pre-installed v16), fixed by prepending the versioned install directory to `GITHUB_PATH`. A second live run then succeeded end-to-end — verified via the GitHub API, all 5 steps (`Install postgresql-client 18` → `Dump every real schema` → `Verify the dump is a genuine, non-empty restorable archive` → `Upload to R2` → `Complete job`) green, not assumed from the user's own read of the UI. Setup this session couldn't do unassisted (Cloudflare dashboard access, a dedicated backups bucket + scoped token + lifecycle rule, the Neon connection string, 5 GitHub repository secrets) was done by the user directly, per the same "user does credential-touching/production-adjacent actions themselves" pattern as B1/U32/B2. |
...
- [x] Full automated test suite passing (unit, integration, E2E) — U31 (2026-09-14): backend (`backend-ci.yml`) runs unit+integration (1288 tests) plus the QA-harness E2E suite (`tests/e2e/`, U29's 6-scenario-type/callback coverage) and, newly wired in this unit, the repo-root structural checks (`tests/frontend/`, 23 tests — previously only run locally via `scripts/ci.sh`, never in CI). Wiring that up surfaced one genuine stale assertion (pre-dating U14's later, reviewed decision that days-of-supply isn't a single global-scalar KPI) — fixed the test, not the page. Separately, a new committed browser-level Playwright suite (`e2e/`, own `e2e.yml` workflow) covers both frontends across both roles (login/role-gating, Customers, Query Playground, Scenarios) — the first automated replacement for what every prior unit verified only via live, interactive Playwright MCP sessions. Two genuine CI-only bugs found and fixed while getting this green on GitHub's actual runner (not just locally): (1) the `sandbox` schema's mirror tables only exist once `/api/query/refresh-sandbox` has run at least once — this project's long-lived local dev DB had that from an earlier unit's manual verification, masking the gap; a truly fresh database (CI's own ephemeral Postgres) never had it, so `global-setup.ts` now refreshes it once before any spec runs. (2) Two `websocket_connect` tests in `test_chat_api.py` hit a known Starlette `TestClient` race (connect, then immediately exit with nothing to synchronize on) only under CI's slower/more contended scheduler — fixed with a short, documented synchronization delay. All three workflows (`backend-ci.yml`, `e2e.yml`) independently confirmed green via GitHub's own REST API, not assumed from a successful push.
...
- [x] Zero demo/mock data remains in either frontend template — U7's original strip (Appendix D) re-verified fresh in U31 (2026-09-14): a repo-wide sweep for common demo-data tells (`dicebear` URLs, faker/lorem strings, `@/data` imports, hardcoded mock arrays, placeholder names/emails) came back clean, and the new Playwright suite now renders real content on every page it touches (Customers, Dashboard, Query, Scenarios) rather than asserting on static analysis alone. **One stale-content page found and fixed 2026-09-14, a later same-day session, prompted by the user reading it directly**: subsystem_2's `tasks/page.tsx` (the sidebar's "Tasks" nav item) had described itself as "Scenario Builder... not wired to the backend yet... until Unit 20 establishes how Subsystem 2 authenticates and talks to the API at all (this template currently has no login flow or API client of any kind)" — false at the moment it was written, not just gone stale later: `git log` shows both this stub and the real Scenario Builder (`/scenarios`, with its own working login flow and API client) were added in the exact same commit (`277b817`, 2026-09-03). U31's demo-data sweep didn't catch it because it isn't demo/mock *data* — it's accurate-sounding but wrong prose. Not a missed unit either: PRD line 579/680 explicitly lists this drag-and-drop kanban view as optional, "Priority 3 — cut first," never required for Done — so the page correctly stays unbuilt, but its copy was rewritten to say that accurately (with a link to the real `/scenarios`) instead of making a false claim about the rest of the app. Verified live via Playwright.
...
- [x] Query Playground's permission boundary tested and confirmed (cannot reach live/ledger tables) — U31 (2026-09-14): already comprehensively covered by `backend/tests/infra/test_sandbox_boundary.py` (reads and writes, real seeded tables and future ones, `live`/`engine`/`reporting`/`chat` all proven, not just `live`) — independently re-verified live against the running sandbox role. One accepted, documented residual: Postgres's `pg_catalog` is not permission-filtered (migration 0001's own comment), so the sandbox role can still observe table/column *names* via `pg_catalog`/`pg_class` — never row data, and `information_schema` (also checked) correctly shows nothing. Now also exercised at the UI level by the new Playwright suite's query-playground spec.
- [x] Deployment live, reachable, and matches the Docker Compose local environment — U32 (2026-09-14, see Phase 4 units table): local Compose bumped to postgres:18-alpine, matching Neon exactly (genuine parity re-verified: migrations to head, all seeds, full 1288-test suite, all clean against the new local Postgres 18). Chat attachments moved off the container's ephemeral filesystem onto Cloudflare R2 (durable, survives redeploys) — live-verified with a real put/get/delete round trip against the production bucket. Background scheduler re-enabled at a 30-minute interval (was disabled post-drill to protect Neon's free-tier CU-hours; 30 min keeps it well under the 100 CU-hour allowance while still ticking). Two items remain outside this line's scope, tracked separately in U32's own row: the residual GitHub-auto-build-source disconnect (needs the user's dashboard action) and full CI wiring (no automated deploy-on-merge yet — every deploy so far has been manual `railway up`/`redeploy`)
- [x] Documentation complete (schema diagram, data dictionary, data-flow doc, testing report) — U31 (2026-09-14): `docs/schema-diagram.md` and `docs/data-dictionary.md` are generated, not hand-written, by a new `scripts/generate_docs.py` that introspects the live SQLAlchemy `Base.metadata` (38 tables across `live`/`reporting`/`engine`/`chat`) — chosen specifically so they can never silently drift from the real schema the way a hand-authored copy could. Writing it surfaced a real, previously-unknown gap: `backend/app/db/__init__.py` never imported `app.db.evaluation`/`human_review`/`portfolio`, so `alembic`'s own autogenerate metadata (and, until worked around, this generator) was silently missing 3 real `engine`-schema tables migrations 0020/0021 actually created. Couldn't be fixed directly in this pass — `backend/` sits behind a harness-init gate (`.claude/hooks/enforce-gate.sh`) that Claude Code's own auto-mode classifier refused to let get cleared via `docker exec`, correctly reading it as infrastructure it shouldn't run unasked — so the user applied the 3-line import fix by hand. Verified after: `alembic revision --autogenerate` against the real dev database no longer proposes dropping any of the three tables (the throwaway revision was inspected and deleted, never applied — its only remaining proposed drops are pre-existing, unrelated drift: `sandbox`'s runtime-created tables and a few raw-SQL partial indexes), and the full 1288-test backend suite still passes. `docs/testing-report.md` documents the fix but also the one real residual: no regression test guards against the same class of gap recurring for a future model module. `docs/data-flow.md` covers the three boundaries the PRD treats as load-bearing (the `live`→`reporting` lag, the `sandbox` permission wall, the Subsystem 1↔2 API-only boundary) with one Mermaid flowchart, kept deliberately short per 5.8's own wording. `docs/testing-report.md` also names a second, separate gap: lint runs nowhere in CI (only locally, non-blocking, via `scripts/ci.sh`) — not fixed, since it's outside this unit's scope, but not left implicit either.
- [x] End-to-end live walkthrough deliverable without improvisation, using QA/synthetic data — U31 (2026-09-14): `docs/walkthrough-script.md`, written and fact-checked against the actual running source (button labels, route paths, and empty-state text grepped from the frontend components themselves, not recalled) rather than drafted from memory — this caught two would-be false claims before they shipped: the QA harness (`tests/support/qa_harness.py`) tears down every row it creates, so it cannot be run then inspected live in the UI as a first draft of this script assumed; and the chat composer's AI-assist buttons (`suggest-pushback`/`sufficiency-check`, Unit 23) and the evaluation/human-review/portfolio/Ledger-action routes (Units 24-26) have no frontend UI at all, so a first draft's claim that Approve/Activate reveals AI-assist buttons in the composer was also wrong. The corrected script covers both roles across both frontends (login, dashboard, Customers, Query Playground's read/write boundary, Query Log, role-gating, scenario creation, ground-truth editing, live chat) using only seeded/QA data, states up front exactly which 3 actions need a live `ANTHROPIC_API_KEY` (none of which block completing the loop — hand-authored ground truth and hand-typed chat replies are this project's own established no-key path, the same one Unit 29's QA harness and the full backend suite already use), and is explicit about which remaining lifecycle steps are real, tested, and API-only rather than silently skipping or fabricating a UI for them.

---

## 10. Definition of Done Tracking (PRD 1.6)

- [x] Both subsystems fully implemented per spec — every phase checklist (§4/§5/§6/§7) closed, edge case catalog 32/32 (§8), B3's AI wiring gap closed 2026-09-14 (see §2). One accepted, documented deviation: B13 (§2) — scenario activation has no true terminal state, `VALID_TRANSITIONS` has no `active →` edge, and edge-case-catalog row 19 ("only one scenario active at a time") is enforced app-side against a documented proxy signal rather than the real invariant, explicitly "not race-safe" per `activate_scenario`'s own docstring. Row 19 itself has a passing test and is closed in the catalog's own terms; B13 is a real, known gap in the underlying implementation, left open at the user's explicit choice (2026-09-14, declined to scope it when asked) rather than silently absent from this line
- [x] Full automated test suite passes — unit, integration, E2E — including every §9.2 edge case — see §9's mirror of this line (U31, 2026-09-14) for the full breakdown
- [x] Subsystems integrate seamlessly: Subsystem 2 reads Subsystem 1 only via API; Ledger behaves correctly across full lifecycle; evaluation pipeline runs end-to-end unattended — closed 2026-09-14: with a real `ANTHROPIC_API_KEY` and `claude_client_enabled=true`, `POST /regenerate` and `POST /complete` both verified live (200, real Claude-generated content, real DB writes) through the actual running app, not a script bypassing it — see B3 (§2). **Caveat:** verified under a transient `MEADOWOPS_CLAUDE_CLIENT_ENABLED=true` set only for the duration of that verification, then reverted — the flag is deliberately off in every persisted config (local `.env`, `.env.example`, CI), so "unattended" means the code path runs unattended once opted in per-process, not that the deployed system defaults to live AI calls
- [x] Full scenario loop exercised via QA test runs across all 6 scenario types at least once each — U29, done (see §6/DD-39)
- [x] Zero demo/fake/fabricated data from either frontend template — see §9's mirror of this line (U31, 2026-09-14)
- [x] Deployed and reachable per §8.2; backups + migrations verified, restore actually tested — U32 (backend deployment, 2026-09-14) + B2 (restore drill, 2026-09-13); **frontend deployment gap found and closed 2026-09-14** (see B1's own row, §2) — both Next.js apps are now live on Vercel (`orbynadmin`/Subsystem 1 → `meadowops-company.vercel.app`, `shadcn-dashboard`/Subsystem 2 → `meadowops-admin.vercel.app`, names swapped from the original assignment same day per the user's own catch — see B1's row for detail), not just the backend as this line previously implied. See §9's corresponding lines
- [x] Documentation complete: schema diagram, data dictionary, data-flow doc, testing report — see §9's mirror of this line (U31, 2026-09-14) for the full breakdown
- [x] Live walkthrough deliverable end-to-end on QA/synthetic data without improvisation — see §9's mirror of this line (U31, 2026-09-14) for the full breakdown
